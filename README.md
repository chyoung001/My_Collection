...
> **스포츠/트레이딩 카드 컬렉션 가치 추적 및 관리 웹 애플리케이션**
>
...
**My Collection**은 보유 중인 스포츠 카드를 체계적으로 관리하고 가치 변화를 추적하는 웹 서비스입니다.
**PSA Public API**로 카드 정보를 자동 등록하고, **130point.com**(eBay 낙찰가 집계)을 스크래핑해 시세를 수집하며,
수집한 시세로 포트폴리오 총액과 추이를 한눈에 보여 줍니다. 카드를 프레임에 진열하는 **갤러리 큐레이션**도 지원합니다.
...
| `frontend` | React + Vite 프론트엔드 (루트에 프론트 코드) |

각 브랜치 루트의 `README.md`에 상세 설치·실행 방법이 있습니다.
...
> 포트폴리오 가치 추이 차트, Top Performers를 한 화면에서 확인합니다.
...
> 카드 상세에서 **130point.com**을 ZenRows 헤드리스 브라우저로 조회해 최근 낙찰가를 수집합니다.
> 제목 매칭 · 적응형 기간 필터 · IQR 이상치 제거 후 평균/중앙값/대표가와 신뢰도를 산출하고,
> 시계열 스냅샷으로 저장해 가격 이력 차트를 그립니다.
...
- **PostgreSQL** (`pg`) — 부팅 시 자동 마이그레이션
- **Swagger UI** — `/docs`
- **배포:** Railway

### Frontend (`frontend` 브랜치)
- **React 18 · Vite 5**
- **Tailwind CSS 3 · Radix UI** (shadcn 스타일)
- **React Router 6**
...
| **130point.com (ZenRows)** | 최근 낙찰가 스크래핑 → 시세 수집 |
| **Ollama Cloud** | LLM 테스트(실험적, OpenAI 호환 SDK) |
...
|--------|----------|------|:----:|
| GET | `/api/cards` · `/api/cards/:id` | 카드 목록 / 단건 | — |
| POST | `/api/cards` · `/api/cards/auto` | 수동 / PSA 자동 등록 | 🔒 |
| DELETE / PATCH | `/api/cards/:id...` | 삭제 / 필드 수정 | 🔒 |
| GET | `/api/dashboard/{summary,top-cards,top-gainer}` | 대시보드 집계 | — |
| GET | `/api/snapshots/{summary,latest}` · `/:id/history` | 시세 조회 | — |
| POST | `/api/snapshots/:cardId/fetch` | 시세 수집(130point) | 🔒💰 |
