...
// 스키마 마이그레이션 — 앱 부팅 시 멱등(idempotent) 실행.
// ⚠️ 이 배열이 스키마의 단일 소스(source of truth)다. schema.sql은 동일 내용의 수동 적용/참고용 사본이므로,
//    스키마 변경 시 반드시 양쪽을 함께 갱신할 것. 모든 문장은 IF NOT EXISTS / 가드로 멱등하게 작성한다.
const migrations = [
  ...
  // 판매(Sold) — sold_at IS NULL = 보유, 값 있으면 판매됨. sold_price=판매가(USD, net), sold_note=메모.
  ["sold_at", "ALTER TABLE cards ADD COLUMN IF NOT EXISTS sold_at TIMESTAMP WITH TIME ZONE"],
  ["sold_price", "ALTER TABLE cards ADD COLUMN IF NOT EXISTS sold_price NUMERIC(12,2)"],
  ["sold_note", "ALTER TABLE cards ADD COLUMN IF NOT EXISTS sold_note VARCHAR(255)"],
  ...
  // 보유/판매 분리 필터(WHERE sold_at IS NULL / IS NOT NULL)용
  ["idx_cards_sold_at",
...
// 마이그레이션을 순서대로 실행. 각 문장은 멱등하므로 재실행 안전.
// 실패는 warn-only로 두어 한 항목이 깨져도 나머지는 진행한다(부팅 차단 방지). 신규 항목은 배열 끝에 추가.
